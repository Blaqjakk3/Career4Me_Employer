        ...
        const loadJobData = async () => {
            ...
            try {
                ...
                setFormData({
                    ...
                    allowCareer4MeApplications: job.allowCareer4MeApplications || false,
                });
                // Initialize validation messages for expiryDate on load
                if (job.expiryDate) {
                    if (!isValidExpiryDate(job.expiryDate)) {
                        const selected = new Date(job.expiryDate);
                        ...
                        setExpiryDateError('');
                        setTimeRemainingMessage(`This job will be live for ${formatTimeRemaining(job.expiryDate)}.`);
                    }
                }
                setLoading(false);
            } catch (error) {
                        ...
                        <form action={formAction} className="p-8 space-y-8">
                            ...
                            <div className="pt-8 border-t border-gray-200">
                                <button
                                    ...
                                    className="w-full px-8 py-4 bg-gradient-to-r from-orange-600 via-red-600 to-pink-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200 text-lg disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    ✨ Update Job Posting
                                </button>
                            </div>
